...
class CChoсolateSlice : public CCondimentDecorator
{
public:
	CChoсolateSlice(IBeveragePtr&& beverage, unsigned count)
		: CCondimentDecorator(move(beverage))
		, m_count(count)
	{
		assertNotExeededMaxCountSlice(count);
	}
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return "Chokolate slice x " + std::to_string(m_count);
	}

private:
	unsigned m_count;
	static constexpr unsigned MAX_COUNT_SLICES = 5;

	static void assertNotExeededMaxCountSlice(unsigned count)
	{
		if (count > MAX_COUNT_SLICES)
		{
			throw std::runtime_error("Exeeded max count chocolate slices");
		}
	}
};
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return GeLiquorTypeName(m_type) + " liquor";
	}

private:
	LiquorType m_type;

	static std::string GeLiquorTypeName(LiquorType type)
	{
		switch (type)
		{
		...
		case LiquorType::Walnut:
			return "Walnut";
		default:
			throw std::runtime_error("Unknown LuquorType");
		}
	}
};